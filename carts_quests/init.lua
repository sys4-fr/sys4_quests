...
if minetest.get_modpath("minetest_quests")
	and (minetest.get_modpath("carts")
			  or minetest.get_modpath("boost_cart")
	)
then
	local S
	...
	if minetest.get_modpath("moreores_quests") then
		ins(
			quests,
			{
				...
				"Rail Installer",
				nil,
				{"default:rail", "moreores:copper_rail"},
				24,
				{mod..":cart", mod..":brakerail"},
				"iron_digger_expert|unlock_copper_rail",
				type = t,
				custom_level = true,
				group = metal
			})
